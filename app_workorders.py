...
# ---------- Cross-Reference (GitHub XLSM) helpers ----------
# GitHub config via secrets (for Parts crossreference.xlsm)
GH_OWNER  = st.secrets.get("GH_OWNER", "")
GH_REPO   = st.secrets.get("GH_REPO", "")
GH_BRANCH = st.secrets.get("GH_BRANCH", "main")
GH_XREF_PATH = st.secrets.get("GH_XREF_PATH", st.secrets.get("GH_WORKBOOK_PATH", "Parts crossreference.xlsm"))
GH_TOKEN  = st.secrets.get("GH_TOKEN", "")

# XRef sheet names & columns
...
XREF_BRAND_PRIORITY = st.secrets.get(
    "XREF_BRAND_PRIORITY",
    ["Caterpillar", "Donaldson", "Fleetguard", "Baldwin", "Wix", "Fram"]
)

_NBSP = "\xa0"

# ---- normalization helpers (prefixed) ----
def xref_normalize_pn(pn) -> str:
    if pd.isna(pn): return ""
    s = str(pn).upper().replace(_NBSP, " ")
...
def xref_norm_text(s: str) -> str:
    if s is None: return ""
    s = str(s).upper().strip().replace(_NBSP, " ")
    s = re.sub(r"[^A-Z0-9 ]+", " ", s)
    s = re.sub(r"\s+", " ", s)
    return s

# ---- GitHub fetch (raw) ----
def _xref_gh_headers_raw():
    h = {"Accept": "application/vnd.github.raw"}
    if GH_TOKEN: h["Authorization"] = f"token {GH_TOKEN}"
    return h

@st.cache_data(ttl=180, show_spinner=False)
def xref_fetch_workbook_bytes(owner: str, repo: str, branch: str, path: str) -> bytes:
    if not owner or not repo or not path:
        raise RuntimeError("Missing GitHub secrets GH_OWNER / GH_REPO / GH_XREF_PATH.")
    url = f"https://api.github.com/repos/{owner}/{repo}/contents/{path}?ref={branch}"
    r = requests.get(url, headers=_xref_gh_headers_raw(), timeout=60)
    if r.status_code == 200 and r.content:
        return r.content
    raw = f"https://raw.githubusercontent.com/{owner}/{repo}/{branch}/{path}"
    r2 = requests.get(raw, timeout=60)
    r2.raise_for_status()
    return r2.content

# ---- Excel loader (supports .xlsm) ----
def xref_load_xlsx_from_bytes(b: bytes) -> pd.ExcelFile:
...
@st.cache_data(ttl=180, show_spinner=False)
def xref_load_inventory_from_bytes(xls_bytes: bytes, debug=False) -> pd.DataFrame:
    xls = xref_load_xlsx_from_bytes(xls_bytes)
...
# ---- Merge sheet ----
def xref_find_header_row_for_merge(df_raw: pd.DataFrame) -> int:
...
@st.cache_data(ttl=180, show_spinner=False)
def xref_load_merge_from_bytes(xls_bytes: bytes, debug=False):
    xls = xref_load_xlsx_from_bytes(xls_bytes)
    df_raw = pd.read_excel(xls, sheet_name=XREF_SHEET_MERGE, header=None)

    hdr_idx = xref_find_header_row_for_merge(df_raw)
...
def xref_apply_brand_priority(df_hits: pd.DataFrame) -> pd.DataFrame:
    if df_hits.empty or not XREF_BRAND_PRIORITY:
        return df_hits
    prio = {b.upper(): i for i, b in enumerate(XREF_BRAND_PRIORITY)}
    return df_hits.assign(_p=df_hits["Brand"].str.upper().map(prio).fillna(999)).sort_values(by=["_p","PartNumber","RowID"]).drop(columns="_p")
...
def xref_resolve_rowset_and_primary_brand(query: str, long_df: pd.DataFrame):
    ...
    for cand in buckets:
        if not cand.empty:
            ...
            c = xref_apply_brand_priority(cand).assign(_len=cand["PartNumber"].astype(str).str.len())
...
def xref_build_crossrefs_from_rows(merge_df: pd.DataFrame, row_ids: set) -> pd.DataFrame:
    ...
    xrefs = xrefs.sort_values(by=["PN_Norm","_p","PartNumber"]).drop_duplicates(subset=["PN_Norm"], keep="first").drop(columns="_p").reset_index(drop=True)
    return xrefs
...
def xref_inventory_lookup_per_xref(inv_df: pd.DataFrame, cross_df: pd.DataFrame, location2: str|None) -> pd.DataFrame:
    ...
    for _, xr in cross_df.iterrows():
        brand = str(xr.get("Brand",""))
        pn    = str(xr.get("PartNumber",""))
        if not pn:
            continue
    ...
    agg = result.groupby(group_keys, as_index=False).agg(**{"Matched From": ("Matched Pair", lambda s: ", ".join(sorted({x for x in s if x})))})
...
# ---- Word export for Cross-Ref ----
def xref_make_docx(cross_df: pd.DataFrame, hits_df: pd.DataFrame, query: str, manufacturer: str|None, location2: str|None) -> bytes:
    from docx import Document
    from docx.shared import Pt
    ...
    r = title.add_run("Cross-Reference Finder"); r.bold = True; r.font.size = Pt(16)
    meta = doc.add_paragraph()
    meta.add_run("Query: ").bold = True; meta.add_run(query)
    meta.add_run("   |   Manufacturer: ").bold = True; meta.add_run(manufacturer or "Unknown")
    if location2: meta.add_run("   |   Location2: ").bold = True; meta.add_run(str(location2))
    doc.add_paragraph("")

    doc.add_paragraph("Cross-References").runs[0].bold = True
    cr_cols = ["Brand","PartNumber"]
    t1 = doc.add_table(rows=1, cols=len(cr_cols))
    for i,c in enumerate(cr_cols): t1.rows[0].cells[i].paragraphs[0].add_run(c).bold = True
    for _,row in cross_df[cr_cols].iterrows():
        cells = t1.add_row().cells
        for i,c in enumerate(cr_cols): cells[i].text = str(row[c]) if pd.notna(row[c]) else ""
    doc.add_paragraph("")

    doc.add_paragraph("Inventory Matches").runs[0].bold = True
    if hits_df.empty:
        doc.add_paragraph("No stock found.")
    else:
        possible_cols = [
            "Matched From","Matched Brand","Matched PN","Inventory PN","Name","Location2","Location","Area","Qty","Stock"
        ]
        cols2 = [c for c in possible_cols if c in hits_df.columns]
        chosen = []
        seen_groups = set()
        for c in cols2:
            g = {"Matched From":"gMF","Matched Brand":"gMF","Matched PN":"gMF"}.get(c, c)
            if g in seen_groups:
                continue
            seen_groups.add(g)
            chosen.append(c)
        t2 = doc.add_table(rows=1, cols=len(chosen))
        for i,c in enumerate(chosen): t2.rows[0].cells[i].paragraphs[0].add_run(c).bold = True
        for _,row in hits_df[chosen].iterrows():
            cells = t2.add_row().cells
            for i,c in enumerate(chosen): cells[i].text = str(row[c]) if pd.notna(row[c]) else ""
...
if auth_status is False:
    ...
    if page == "🔁 Cross Reference":
        ...
        try:
            xls_bytes = xref_fetch_workbook_bytes(GH_OWNER, GH_REPO, GH_BRANCH, GH_XREF_PATH)
            inv_df = xref_load_inventory_from_bytes(xls_bytes, debug=xref_debug)
            merge_df, long_df = xref_load_merge_from_bytes(xls_bytes, debug=xref_debug)
        except Exception as e:
            st.error(f"Cross-Reference: GitHub fetch failed: {e}")
            st.stop()
        ...
        if st.button("Find"):
            ...
            try:
                docx_bytes = xref_make_docx(xrefs, hits, pn_input, primary_brand, sel_loc2 if use_loc else None)
                st.download_button(
                    "⬇️ Download Word",
                    data=docx_bytes,
                    file_name=f"XRef_{xref_normalize_pn(pn_input)}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                )
            except Exception:
                st.caption("Install python-docx to enable Word export: `pip install python-docx`")
        st.stop()
