# app_workorders.py — Parquet fast‑path + External Links (CrossRef & SP moved out)
# ---------------------------------------------------------------------------------
...
#   - 🧰 Service Procedures → https://service-procedure-filter.streamlit.app/
#   - 🔁 Cross Reference → https://crossreference.streamlit.app/
#
...
APP_VERSION = "2025.10.16-extlinks"
...
st.markdown(
    ...
    <style>
      ...
      .extlink a { 
        ...
        border:1px solid rgba(49,51,63,.2); text-decoration:none;
      }
    </style>
    """,
...
# External links only (replace in-app pages for CrossRef & Service Procedures)
def _ext_link(label: str, url: str):
    # Try Streamlit 1.25+ link_button; fallback to HTML anchor that opens new tab.
    try:
        st.sidebar.link_button(label, url)  # type: ignore[attr-defined]
    except Exception:
        st.sidebar.markdown(f'<div class="extlink"><a href="{url}" target="_blank">{label}</a></div>', unsafe_allow_html=True)

_ext_link("🧰 Service Procedures ↗", "https://service-procedure-filter.streamlit.app/")
_ext_link("🔁 Cross Reference ↗", "https://crossreference.streamlit.app/")

cfg = (lambda c: to_plain(c))((lambda: (to_plain(st.secrets["app_config"]) if "app_config" in st.secrets else load_config()))())
...
if auth_status is False:
    ...
    xmtime = os.path.getmtime(xlsx_path)
    ...
    if page == "🧾 Service Report":
        ...
        with t_over:
            ...
            if overdue_df.empty:
                ...
                show = overdue_df[[c for c in base if c in show.columns]].copy()
